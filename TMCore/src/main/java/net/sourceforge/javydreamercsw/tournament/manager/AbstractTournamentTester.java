...
public abstract class AbstractTournamentTester {
    ...
    private void printPairings(Map<Integer, Encounter> result) {
        for (Map.Entry<Integer, Encounter> entry : result.entrySet()) {
    ...
    public void testSimulateTournament() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            while (instance.getAmountOfTeams() > 1) {
                try {
                    ...
                    if (instance.getActiveTeams().size() > 1) {
                        ...
                        for (Map.Entry<Integer, Encounter> entry : instance.getPairings().entrySet()) {
                            Encounter encounter = entry.getValue();
                            ...
                            if (!player1.equals(TournamentInterface.bye) && !player2.equals(TournamentInterface.bye)) {
                                //Random Result
                            ...
                            if (player1.equals(TournamentInterface.bye) || player2.equals(TournamentInterface.bye)
                                    && instance.getActiveTeams().size() == 1) {
            ...
            for (Map.Entry<Integer, List<TeamInterface>> rankings : instance.getRankings().entrySet()) {
