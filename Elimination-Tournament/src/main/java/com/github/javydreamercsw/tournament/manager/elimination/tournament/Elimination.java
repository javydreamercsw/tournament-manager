 ...
 */
public abstract class Elimination extends AbstractTournament implements TournamentInterface {

  private static final Logger LOG = Logger.getLogger(Elimination.class.getName());

  /**
   * Provide eliminations and pairing option. Defaults to 3 points for a win, 0 for loses and 1 for
   * draws.
   *
  ...
  /**
   * Provide eliminations and pairing option. Defaults to one elimination, 3 points for a win, 0 for
   * loses, 1 for draws and random pairing.
   */
  public Elimination() {
    super(3, 0, 1, false);
  }
   ...
   */
  public Elimination(
      int eliminations, int winPoints, int lossPoints, int drawPoints, boolean pairAlikeRecords) {
  ...
  public Map<Integer, Encounter> getPairings() {
    ...
    return pairingHistory.getOrDefault(getRound(), new HashMap<>());
  }

  @Override
  public int getMinimumAmountOfRounds() {
    /**
     * Assuming two competitors per match, if there are n competitors, there will be r = log {2} n
     * rounds required, or if there are r rounds, there will be n= 2^r competitors. In the opening
     * round, 2^r - n competitors will get a bye.
     */
