...
/**
 * Swiss tournament is an elimination tournament without eliminations. Just finishes when there's
 * one team above the rest by the win point amount or more.
 *
...
public class Swiss extends AbstractTournament implements TournamentInterface {
  ...
  private static final Logger LOG = Logger.getLogger(Swiss.class.getSimpleName());
  ...
  public Map<Integer, Encounter> getPairings() {
    ...
    if (rankings.size() >= 2 && round > 1) {
      ...
      if (highRank != null && highRank.size() == 1) {
        ...
        LOG.log(
            Level.INFO,
            "Multiple teams tied at ranking 1: {0}",
