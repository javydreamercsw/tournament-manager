...
public class MatchResultJpaController extends AbstractController implements Serializable {
  ...
  public void create(MatchResult matchResult) throws PreexistingEntityException, Exception {
    ...
    try {
      ...
      for (MatchHasTeam matchHasTeamListMatchHasTeam : matchResult.getMatchHasTeamList()) {
        MatchResult oldMatchResultOfMatchHasTeamListMatchHasTeam =
            matchHasTeamListMatchHasTeam.getMatchResult();
        ...
        if (oldMatchResultOfMatchHasTeamListMatchHasTeam != null) {
          oldMatchResultOfMatchHasTeamListMatchHasTeam
              .getMatchHasTeamList()
              .remove(matchHasTeamListMatchHasTeam);
          oldMatchResultOfMatchHasTeamListMatchHasTeam =
              em.merge(oldMatchResultOfMatchHasTeamListMatchHasTeam);
        }
      }
  ...
  public void edit(MatchResult matchResult) throws NonexistentEntityException, Exception {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      MatchResult persistentMatchResult =
          em.find(MatchResult.class, matchResult.getMatchResultPK());
      ...
      if (matchResultTypeNew != null) {
        matchResultTypeNew =
            em.getReference(matchResultTypeNew.getClass(), matchResultTypeNew.getId());
      ...
      for (MatchHasTeam matchHasTeamListNewMatchHasTeam : matchHasTeamListNew) {
        if (!matchHasTeamListOld.contains(matchHasTeamListNewMatchHasTeam)) {
          MatchResult oldMatchResultOfMatchHasTeamListNewMatchHasTeam =
              matchHasTeamListNewMatchHasTeam.getMatchResult();
          ...
          if (oldMatchResultOfMatchHasTeamListNewMatchHasTeam != null
              && !oldMatchResultOfMatchHasTeamListNewMatchHasTeam.equals(matchResult)) {
            oldMatchResultOfMatchHasTeamListNewMatchHasTeam
                .getMatchHasTeamList()
                .remove(matchHasTeamListNewMatchHasTeam);
            oldMatchResultOfMatchHasTeamListNewMatchHasTeam =
                em.merge(oldMatchResultOfMatchHasTeamListNewMatchHasTeam);
          }
        }
      }
      em.getTransaction().commit();
      ...
      if (msg == null || msg.isEmpty()) {
        ...
        if (findMatchResult(id) == null) {
          throw new NonexistentEntityException(
              "The matchResult with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(MatchResultPK id) throws NonexistentEntityException {
    ...
    try {
      ...
      try {
        ...
        throw new NonexistentEntityException(
            "The matchResult with id " + id + " no longer exists.", enfe);
      }
      MatchResultType matchResultType = matchResult.getMatchResultType();
      if (matchResultType != null) {
