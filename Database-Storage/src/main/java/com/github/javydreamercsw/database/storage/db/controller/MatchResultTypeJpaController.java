...
public class MatchResultTypeJpaController extends AbstractController implements Serializable {
  ...
  public void create(MatchResultType matchResultType) {
    ...
    try {
      ...
      for (MatchResult matchResultListMatchResultToAttach : matchResultType.getMatchResultList()) {
        matchResultListMatchResultToAttach =
            em.getReference(
                matchResultListMatchResultToAttach.getClass(),
                matchResultListMatchResultToAttach.getMatchResultPK());
      ...
      for (MatchResult matchResultListMatchResult : matchResultType.getMatchResultList()) {
        MatchResultType oldMatchResultTypeOfMatchResultListMatchResult =
            matchResultListMatchResult.getMatchResultType();
        ...
        if (oldMatchResultTypeOfMatchResultListMatchResult != null) {
          oldMatchResultTypeOfMatchResultListMatchResult
              .getMatchResultList()
              .remove(matchResultListMatchResult);
          oldMatchResultTypeOfMatchResultListMatchResult =
              em.merge(oldMatchResultTypeOfMatchResultListMatchResult);
        }
      }
  ...
  public void edit(MatchResultType matchResultType)
      throws IllegalOrphanException, NonexistentEntityException, Exception {
    EntityManager em = null;
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      MatchResultType persistentMatchResultType =
          em.find(MatchResultType.class, matchResultType.getId());
      ...
      for (MatchResult matchResultListOldMatchResult : matchResultListOld) {
        if (!matchResultListNew.contains(matchResultListOldMatchResult)) {
          ...
          illegalOrphanMessages.add(
              "You must retain MatchResult "
                  + matchResultListOldMatchResult
                  + " since its matchResultType field is not nullable.");
        }
      }
      if (illegalOrphanMessages != null) {
      ...
      for (MatchResult matchResultListNewMatchResultToAttach : matchResultListNew) {
        matchResultListNewMatchResultToAttach =
            em.getReference(
                matchResultListNewMatchResultToAttach.getClass(),
                matchResultListNewMatchResultToAttach.getMatchResultPK());
      ...
      for (MatchResult matchResultListNewMatchResult : matchResultListNew) {
        if (!matchResultListOld.contains(matchResultListNewMatchResult)) {
          MatchResultType oldMatchResultTypeOfMatchResultListNewMatchResult =
              matchResultListNewMatchResult.getMatchResultType();
          ...
          if (oldMatchResultTypeOfMatchResultListNewMatchResult != null
              && !oldMatchResultTypeOfMatchResultListNewMatchResult.equals(matchResultType)) {
            oldMatchResultTypeOfMatchResultListNewMatchResult
                .getMatchResultList()
                .remove(matchResultListNewMatchResult);
            oldMatchResultTypeOfMatchResultListNewMatchResult =
                em.merge(oldMatchResultTypeOfMatchResultListNewMatchResult);
          }
        }
      }
      em.getTransaction().commit();
      ...
      if (msg == null || msg.isEmpty()) {
        ...
        if (findMatchResultType(id) == null) {
          throw new NonexistentEntityException(
              "The matchResultType with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
    ...
    try {
      ...
      try {
        ...
        throw new NonexistentEntityException(
            "The matchResultType with id " + id + " no longer exists.", enfe);
      }
      List<String> illegalOrphanMessages = null;
      ...
      for (MatchResult matchResultListOrphanCheckMatchResult : matchResultListOrphanCheck) {
        ...
        illegalOrphanMessages.add(
            "This MatchResultType ("
                + matchResultType
                + ") cannot be destroyed since the MatchResult "
                + matchResultListOrphanCheckMatchResult
                + " in its matchResultList field has a non-nullable matchResultType field.");
      }
      if (illegalOrphanMessages != null) {
        throw new IllegalOrphanException(illegalOrphanMessages);
      }
  ...
  private List<MatchResultType> findMatchResultTypeEntities(
