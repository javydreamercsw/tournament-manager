...
public class TeamService extends Service<Team> {
  private TeamJpaController tc = new TeamJpaController(DataBaseManager.getEntityManagerFactory());
  ...
  /**
   * Convert a team form the database representation to the JSKill representation.
   *
   * @param t Team to convert.
   * @param format Format to get ratings for.
   * @return converted team.
   */
  public com.github.javydreamercsw.tournament.manager.Team convertToTeam(Team t, Format format) {
    com.github.javydreamercsw.tournament.manager.Team team =
        new com.github.javydreamercsw.tournament.manager.Team(t.getId(), new ArrayList<>());
    t.getPlayerList()
        .forEach(
            player -> {
              Rating r;
  ...
  public TeamHasFormatRecord getFormatRecord(Team team, Format format) {
    for (TeamHasFormatRecord thfr : findTeam(team.getId()).getTeamHasFormatRecordList()) {
