  ...
  public void create(Game game) {
    ...
    try {
      ...
      for (Format formatListFormatToAttach : game.getFormatList()) {
        formatListFormatToAttach =
            em.getReference(
                formatListFormatToAttach.getClass(), formatListFormatToAttach.getFormatPK());
  ...
  public void edit(Game game) throws IllegalOrphanException, NonexistentEntityException, Exception {
    ...
    try {
      ...
      for (Format formatListOldFormat : formatListOld) {
        if (!formatListNew.contains(formatListOldFormat)) {
          ...
          illegalOrphanMessages.add(
              "You must retain Format "
                  + formatListOldFormat
                  + " since its game field is not nullable.");
        }
      }
      ...
      for (Format formatListNewFormatToAttach : formatListNew) {
        formatListNewFormatToAttach =
            em.getReference(
                formatListNewFormatToAttach.getClass(), formatListNewFormatToAttach.getFormatPK());
    ...
    try {
      ...
      for (Format formatListOrphanCheckFormat : formatListOrphanCheck) {
        ...
        illegalOrphanMessages.add(
            "This Game ("
                + game
                + ") cannot be destroyed since the Format "
                + formatListOrphanCheckFormat
                + " in its formatList field has a non-nullable game field.");
      }
      ...
      for (Record recordListOrphanCheckRecord : recordListOrphanCheck) {
        ...
        illegalOrphanMessages.add(
            "This Game ("
                + game
                + ") cannot be destroyed since the Record "
                + recordListOrphanCheckRecord
                + " in its recordList field has a non-nullable game field.");
      }
      if (illegalOrphanMessages != null) {
        throw new IllegalOrphanException(illegalOrphanMessages);
      }
