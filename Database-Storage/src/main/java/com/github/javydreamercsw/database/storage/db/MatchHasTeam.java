...
@NamedQueries({
  ...
  @NamedQuery(
      name = "MatchHasTeam.findByTeamId",
      query = "SELECT m FROM MatchHasTeam m WHERE m.matchHasTeamPK.teamId = :teamId"),
  @NamedQuery(
      name = "MatchHasTeam.findByMatchEntryId",
      query = "SELECT m FROM MatchHasTeam m WHERE m.matchHasTeamPK.matchEntryId = :matchEntryId"),
  @NamedQuery(
      name = "MatchHasTeam.findByMatchEntryFormatId",
      query =
          "SELECT m FROM MatchHasTeam m WHERE m.matchHasTeamPK.matchEntryFormatId ="
              + " :matchEntryFormatId"),
  @NamedQuery(
      name = "MatchHasTeam.findByMatchEntryFormatGameId",
      query =
          "SELECT m FROM MatchHasTeam m WHERE m.matchHasTeamPK.matchEntryFormatGameId ="
              + " :matchEntryFormatGameId")
})
public class MatchHasTeam implements Serializable {
  private static final long serialVersionUID = 1L;
  @EmbeddedId protected MatchHasTeamPK matchHasTeamPK;
  ...
  @JoinColumns({
    @JoinColumn(
        ...
        updatable = false),
    @JoinColumn(
        name = "match_entry_format_id",
        referencedColumnName = "format_id",
        insertable = false,
        updatable = false),
    @JoinColumn(
        name = "match_entry_format_game_id",
        referencedColumnName = "format_game_id",
        insertable = false,
        updatable = false)
  })
  ...
  @JoinColumns({
    ...
    @JoinColumn(
        name = "match_result_match_result_type_id",
  ...
  public MatchHasTeam(
      int teamId, int matchEntryId, int matchEntryFormatId, int matchEntryFormatGameId) {
    this.matchHasTeamPK =
        new MatchHasTeamPK(teamId, matchEntryId, matchEntryFormatId, matchEntryFormatGameId);
  }
