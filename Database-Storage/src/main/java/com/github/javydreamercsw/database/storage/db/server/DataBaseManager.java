...
public class DataBaseManager {
  ...
  public static final String JNDI_DATASOURCE_NAME = "java:comp/env/jdbc/TMDB";
  ...
  public static void updateDBState() {
    ...
    try {
      ...
      try {
        ...
        ((JdbcDataSource) ds)
            .setPassword((String) getProperties().get("jakarta.persistence.jdbc.password"));
        ((JdbcDataSource) ds)
            .setUser((String) getProperties().get("jakarta.persistence.jdbc.user"));
        ((JdbcDataSource) ds).setURL((String) getProperties().get("jakarta.persistence.jdbc.url"));
        // Load the driver
        Class.forName((String) getProperties().get("jakarta.persistence.jdbc.driver"));
  ...
  private static void initializeFlyway(DataSource dataSource) {
    ...
    if (status == null) {
      ...
      try {
        flyway.baseline(); // Use baseline for initial schema history table creation
        LOG.info("Done!");
  ...
  private static void updateDatabase(DataSource dataSource) {
    ...
    try {
      ...
      LOG.info("Done!");
      setState(
          flyway.info().current().getState() == MigrationState.SUCCESS
              ? DBState.VALID
              : DBState.ERROR);
    } catch (FlywayException fe) {
  ...
  private static List<Object> namedQuery(
      String query, Map<String, Object> parameters, boolean change) {
    EntityTransaction transaction = getEntityManager().getTransaction();
    if (change) {
      transaction.begin();
    }
  ...
  public static void loadDemoData() throws Exception {
    ...
    Game game = fg.orElseGet(() -> new Game(gameAPI.getName()));
    ...
    for (GameFormat format : gameAPI.gameFormats()) {
      ...
      Optional<Format> f =
          FormatService.getInstance().findFormatForGame(gameAPI.getName(), format.getName());
      if (!f.isPresent()) {
        // Let's create it.
    ...
    // Add a tournaments
    List<TournamentInterface> formats =
        new ArrayList<>(Lookup.getDefault().lookupAll(TournamentInterface.class));

    for (TournamentInterface format : formats) {
      TournamentFormat tf =
          new TournamentFormat(format.getName(), format.getClass().getCanonicalName());
    ...
    List<Format> formatList = FormatService.getInstance().findFormatByGame(gameAPI.getName());
    ...
    for (int i = 0; i < 10; i++) {
      ...
      match.setFormat(
          FormatService.getInstance()
              .findFormatById(formatList.get(r.nextInt(formatList.size())).getFormatPK())
              .get());
      MatchService.getInstance().saveMatch(match);
      for (int j = 0; j < 2; j++) {
        while (!MatchService.getInstance().addTeam(match, teams.get(r.nextInt(teams.size())))) {}
      }
      ...
      MatchService.getInstance()
          .setResult(
              match.getMatchHasTeamList().get(0),
              MatchService.getInstance().getResultType(win ? "result.win" : "result.loss").get());
      MatchService.getInstance()
          .setResult(
              match.getMatchHasTeamList().get(1),
              MatchService.getInstance().getResultType(win ? "result.loss" : "result.win").get());
  ...
  public static void load() throws Exception {
    for (TournamentInterface format : Lookup.getDefault().lookupAll(TournamentInterface.class)) {
    ...
    for (IGame gameAPI : Lookup.getDefault().lookupAll(IGame.class)) {
      ...
      Optional<Game> result = GameService.getInstance().findGameByName(gameAPI.getName());
