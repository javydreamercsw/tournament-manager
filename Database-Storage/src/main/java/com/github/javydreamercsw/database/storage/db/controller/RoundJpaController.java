    ...
    try {
        ...
        if (!matchEntryListNew.contains(matchEntryListOldMatchEntry)) {
          ...
          illegalOrphanMessages.add(
              "You must retain MatchEntry "
                  ...
                  + " since its round field is not nullable.");
        }
      }
      if (illegalOrphanMessages != null) {
      ...
      for (MatchEntry matchEntryListNewMatchEntry : matchEntryListNew) {
        if (!matchEntryListOld.contains(matchEntryListNewMatchEntry)) {
          ...
          if (oldRoundOfMatchEntryListNewMatchEntry != null
              && !oldRoundOfMatchEntryListNewMatchEntry.equals(round)) {
            oldRoundOfMatchEntryListNewMatchEntry
                .getMatchEntryList()
    ...
    try {
      ...
      try {
        ...
        throw new NonexistentEntityException(
            "The round with id " + id + " no longer exists.", enfe);
      }
      List<String> illegalOrphanMessages = null;
      ...
      for (MatchEntry matchEntryListOrphanCheckMatchEntry : matchEntryListOrphanCheck) {
        ...
        illegalOrphanMessages.add(
            "This Round ("
                ...
                + " in its matchEntryList field has a non-nullable round field.");
      }
      if (illegalOrphanMessages != null) {
        throw new IllegalOrphanException(illegalOrphanMessages);
      }
