...
public class FormatService extends Service<Format> {
   ...
   * formats. The returned list is ordered by name.
   *
  ...
  /**
   * Searches for the exact category whose name matches the given filter text. The matching is case
   * insensitive.
   *
   ...
   *
   * <p>Behaves like {@link #findFormatByName(String)}, except that returns a {@link Format} instead
   * of an {@link Optional}. If the category can't be identified, an exception is thrown.
   *
   * @param name the filter text
  ...
  public Format findFormatOrThrow(String name) {
    return findFormatByName(name)
        .orElseThrow(() -> new IllegalStateException("Format " + name + " does not exist"));
  }
  ...
  public void saveFormat(Format format) throws Exception {
    if (format.getFormatPK() != null && fc.findFormat(format.getFormatPK()) != null) {
      fc.edit(format);
    } else {
      fc.create(format);
    }
