...
public class AbstractServerTest {
  ...
  protected void cleanDB() throws NonexistentEntityException, IllegalOrphanException {
    ...
    TeamService.getInstance().getAll().forEach(team -> TeamService.getInstance().deleteTeam(team));
    PlayerService.getInstance()
        .getAll()
        .forEach(player -> PlayerService.getInstance().deletePlayer(player));
    FormatService.getInstance()
        .getAll()
        .forEach(format -> FormatService.getInstance().deleteFormat(format));
    GameService.getInstance().getAll().forEach(game -> GameService.getInstance().deleteGame(game));
  }
}
