...
public class TeamServiceTest extends AbstractServerTest {
  ...
  public void testFindTeam() {
    assertNotNull(
        TeamService.getInstance()
            .findTeam(TeamService.getInstance().findTeams("Player 1").get(0).getId()));
  }
  ...
  public void testSaveTeam() {
    ...
    assertEquals(TeamService.getInstance().findTeam(team.getId()).getPlayerList().size(), 0);
    ...
    assertEquals(TeamService.getInstance().findTeam(team.getId()).getPlayerList().size(), 2);
