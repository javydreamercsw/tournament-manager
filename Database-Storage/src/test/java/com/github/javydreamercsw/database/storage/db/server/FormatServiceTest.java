...
public class FormatServiceTest extends AbstractServerTest {
  ...
  public void testFindFormatByName() {
    assertEquals(
        1, FormatService.getInstance().findFormats(game.getFormatList().get(0).getName()).size());
    assertEquals(
        0,
        FormatService.getInstance()
            .findFormats(game.getFormatList().get(0).getName() + "2")
            .size());
  }
  ...
  public void testFindFormatOrThrow() {
    assertNotNull(
        FormatService.getInstance().findFormatOrThrow(game.getFormatList().get(0).getName()));
    try {
      FormatService.getInstance().findFormatOrThrow(game.getFormatList().get(0).getName() + "2");
  ...
  public void testFindFormatById() {
    assertNotNull(
        FormatService.getInstance()
            .findFormatById(
                FormatService.getInstance()
                    .findFormatByName(game.getFormatList().get(0).getName())
                    .get()
                    .getFormatPK()));
  }
  ...
  public void testDeleteFormat() throws Exception {
    ...
    FormatService.getInstance()
        .deleteFormat(FormatService.getInstance().findFormatByName(format.getName()).get());
    assertEquals(FormatService.getInstance().findFormats("test").size(), 0);
  }
  ...
  public void testFindFormatForGame() {
    Optional<Format> result =
        FormatService.getInstance()
            .findFormatForGame(game.getName(), game.getFormatList().get(0).getName());
    assertTrue(result.isPresent());

    result =
        FormatService.getInstance()
            .findFormatForGame(game.getName(), game.getFormatList().get(0).getName() + "x");
    assertFalse(result.isPresent());

    result =
        FormatService.getInstance()
            .findFormatForGame(game.getName() + "x", game.getFormatList().get(0).getName());
    assertFalse(result.isPresent());
  }
