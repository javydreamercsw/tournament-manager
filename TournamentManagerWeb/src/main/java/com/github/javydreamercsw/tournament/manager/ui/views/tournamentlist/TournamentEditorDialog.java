...
public class TournamentEditorDialog extends AbstractEditorDialog<Tournament> {
  ...
  public TournamentEditorDialog(
      BiConsumer<Tournament, Operation> itemSaver, Consumer<Tournament> itemDeleter) {
  ...
  private void addSettings() {
    ...
    dateHour.setItems(minutes.subList(0, 24));
    dateHour.setItemLabelGenerator(Object::toString);
    if (getCurrentItem() != null) {
      dateHour.setValue(
          getCurrentItem().getSignupDate() == null
              ? null
              : getCurrentItem().getSignupDate().getHour());
    }
    dateHour.addValueChangeListener(
        event -> {
          updateSignupDate();
        });

    dateMin.setPlaceholder("Minutes");
    dateMin.setItems(minutes);
    dateMin.setItemLabelGenerator(Object::toString);
    if (getCurrentItem() != null) {
      dateMin.setValue(
          getCurrentItem().getSignupDate() == null
              ? null
    ...
    dateMin.addValueChangeListener(
        event -> {
          updateSignupDate();
        });
    ...
    signups.addValueChangeListener(
        event -> {
          updateSignupDate();
        });
    getBinder()
        .forField(signups)
        .withConverter(
            new Converter<LocalDate, LocalDateTime>() {
              ...
              public Result<LocalDateTime> convertToModel(LocalDate value, ValueContext context) {
                LocalTime lt =
                    LocalTime.of(
                        dateHour.getValue() == null ? 0 : dateHour.getValue(),
                        dateMin.getValue() == null ? 0 : dateMin.getValue());
    ...
    signupLength.setItems(minutes);
    signupLength.setItemLabelGenerator(Object::toString);

    getFormLayout().add(signupLength);
    getBinder()
        .forField(signupLength)
    ...
    roundLength.setItems(minutes);
    roundLength.setItemLabelGenerator(Object::toString);

    getFormLayout().add(roundLength);
    getBinder()
        .forField(roundLength)
  ...
  private void addNameField() {
    ...
    getBinder()
        .forField(tournamentNameField)
        .withConverter(String::trim, String::trim)
        .withValidator(
            new StringLengthValidator(
                "Tournament name must contain at least 3 printable characters", 3, null))
        .withValidator(
            name -> {
              boolean valid = true;
              for (Tournament t : TournamentService.getInstance().findTournaments(name)) {
                if (t.getName().equals(name)
                    && !t.getTournamentPK().equals(getCurrentItem().getTournamentPK())) {
  ...
  private void addTournamentFormat() {
    ...
    List<TournamentFormat> formats = new ArrayList<>(TournamentService.getInstance().getFormats());
    ...
    getBinder()
        .forField(format)
        .bind(Tournament::getTournamentFormat, Tournament::setTournamentFormat);
  }
  ...
  protected boolean isValid() {
    ...
    //    }
    // All are zero
    return getCurrentItem().getDrawPoints()
            + getCurrentItem().getLossPoints()
            + getCurrentItem().getWinPoints()
        > 0;
  }
