...
public class ResultForm extends FormLayout {
        ...
        .addColumn(
            new ComponentRenderer<>(
                (mht) -> {
                  ...
                  cb.setItems(MatchService.getInstance().getResultTypes());
    ...
    lock.addClickListener(
        event -> {
          MatchService.getInstance()
              .findMatch(entry.getMatchEntryPK())
              .getMatchHasTeamList()
              .forEach(
                  (mht) -> {
                    try {
  ...
  private void validate() {
    boolean valid = true;
    if (entry != null) {
      for (MatchHasTeam mht :
          MatchService.getInstance().findMatch(entry.getMatchEntryPK()).getMatchHasTeamList()) {
