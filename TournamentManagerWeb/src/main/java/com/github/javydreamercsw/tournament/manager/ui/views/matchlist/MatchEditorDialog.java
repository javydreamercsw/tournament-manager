...
public final class MatchEditorDialog extends AbstractEditorDialog<MatchEntry> {
  ...
  public MatchEditorDialog(
      BiConsumer<MatchEntry, Operation> itemSaver, Consumer<MatchEntry> itemDeleter) {
  ...
  private void addTeams() {
    ...
    grid.addColumn(Team::getName).setHeader("Name").setWidth("8em").setResizable(true);
  ...
  private void addDate() {
    getBinder().forField(datePicker).bind(MatchEntry::getMatchDate, MatchEntry::setMatchDate);
                  ...
                  (mht) -> {
                    try {
                      if (mht.getMatchResult() != null) {
                        MatchService.getInstance().setRanked(getCurrentItem(), ranked.getValue());
                      }
                    } catch (Exception ex) {
  ...
  private class TeamSelectionListener
      implements ValueChangeListener<ComponentValueChangeEvent<Checkbox, Boolean>> {
