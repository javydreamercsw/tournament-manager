...
public class PlayerList extends TMView {
  ...
  private final PlayerEditorDialog form =
      new PlayerEditorDialog(this::savePlayer, this::deletePlayer);
  ...
  private void addContent() {
    ...
    grid.addColumn(this::getWinCount).setHeader("Wins").setWidth("6em");
    grid.addColumn(this::getLossCount).setHeader("Losses").setWidth("6em");
    grid.addColumn(this::getDrawCount).setHeader("Draws").setWidth("6em");
  ...
  private Button createEditButton(Player player) {
    Button edit =
        new Button("Edit", event -> form.open(player, AbstractEditorDialog.Operation.EDIT));
  ...
  private void savePlayer(Player player, AbstractEditorDialog.Operation operation) {
    try {
      ...
      Notification.show(
          "Player successfully " + operation.getNameInText() + "ed.", 3000, Position.BOTTOM_START);
      updateView();
    } catch (Exception ex) {
      Exceptions.printStackTrace(ex);
      Notification.show("Unable to save player!", 3000, Position.BOTTOM_START);
    }
  }

  private void deletePlayer(Player player) {
    List<Player> matchesInCategory = PlayerService.getInstance().findPlayers(player.getName());

    if (!matchesInCategory.isEmpty() && player.getTeamList().size() <= 1) {
      PlayerService.getInstance().deletePlayer(player);

      Notification.show("Player successfully deleted.", 3000, Position.BOTTOM_START);
      updateView();
    } else {
      Notification.show("Unable to delete player!", 3000, Position.BOTTOM_START);
      if (player.getTeamList().size() > 1) {
        ...
        Notification.show(
            "This payer is member of multi-person teams that need "
