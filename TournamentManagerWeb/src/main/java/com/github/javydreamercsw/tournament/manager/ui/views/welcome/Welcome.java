...
public class Welcome extends TMView {
  @Serial private static final long serialVersionUID = 1252548231807630022L;

  static {
    try {
      InitialContext context = new InitialContext();

      String JNDIDB = (String) context.lookup("java:comp/env/tm/JNDIDB");

      DataBaseManager.setPersistenceUnitName(JNDIDB);
      boolean demo = (Boolean) context.lookup("java:comp/env/tm/demo");

      // Check if it's configured for demo
      if (demo && PlayerService.getInstance().getAll().isEmpty()) {
        try {
          Notification.show("Loading demo data...", 3000, Position.MIDDLE);

          DataBaseManager.loadDemoData();

          Notification.show("Loading demo data done!", 3000, Position.MIDDLE);
        } catch (Exception ex) {
          Notification.show("Error loading demo data!", 3000, Position.MIDDLE);
        ...
        Notification.show("Loading data done!", 3000, Position.MIDDLE);
      }
  ...
  public Welcome() {
    ...
    List<IGame> games = new ArrayList<>(Lookup.getDefault().lookupAll(IGame.class));
    ...
    cb.addValueChangeListener(
        new ValueChangeListener() {
          @Serial private static final long serialVersionUID = 5377566605252849942L;
  ...
  private static class GameLabelGenerator implements ItemLabelGenerator<IGame> {
    @Serial private static final long serialVersionUID = -4396467477758231860L;
