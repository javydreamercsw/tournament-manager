...
/**
 * Displays the list of available formats, with a search filter as well as buttons to add a new
 * format or edit existing ones.
 */
...
public class FormatList extends TMView {
  ...
  private final FormatEditorDialog form =
      new FormatEditorDialog(this::saveFormat, this::deleteFormat);
  ...
  private void addSearchBar() {
    ...
    newButton.addClickListener(e -> form.open(new Format(), AbstractEditorDialog.Operation.ADD));
  ...
  private void addContent() {
    ...
    grid.addColumn(Format::getName).setHeader("Name").setWidth("8em").setResizable(true);
    grid.addColumn(this::getMatchCount).setHeader("Matches").setWidth("6em");
  ...
  private Button createEditButton(Format category) {
    Button edit =
        new Button("Edit", event -> form.open(category, AbstractEditorDialog.Operation.EDIT));
  ...
  private String getMatchCount(Format category) {
    List<MatchEntry> matchesInCategory =
        MatchService.getInstance().findMatchesWithFormat(category.getName());
  ...
  public void updateView() {
    ...
    FormatService.getInstance()
        .findFormatByGame((String) getValue(CURRENT_GAME))
        .forEach(
            format -> {
  ...
  private void saveFormat(Format format, AbstractEditorDialog.Operation operation) {
    try {
      FormatService.getInstance().saveFormat(format);

      Notification.show(
          "Format successfully " + operation.getNameInText() + "ed.", 3000, Position.BOTTOM_START);
      updateView();
    } catch (Exception ex) {
      Exceptions.printStackTrace(ex);
      Notification.show("Unable to save format!", 3000, Position.BOTTOM_START);
    }
  }
  ...
  private void deleteFormat(Format category) {
    ...
    if (matchesInCategory.isEmpty()) {
      ...
      Notification.show("Unable to delete format!", 3000, Position.BOTTOM_START);
    }
  }
}
